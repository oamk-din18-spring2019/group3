...
.paywall__auth__row{width:804px;min-height:600px;display:flex;flex-direction:row;justify-content:center}
.paywall__auth__col_left{width:100%}
.paywall__auth__col_right{flex:0 0 300px;max-width:300px}
.paywall__auth__image-wrap.login{margin-left:0;margin-bottom:0;margin-right:15px;margin-top: 73px}
.paywall__auth__image-wrap{margin-left:-90px;margin-bottom:45px;margin-right:20px;margin-top: 125px}
.paywall__auth__image{display:block;max-width:100%}
.paywall__auth__main-text{margin-left:15px;margin-bottom:15px;max-width:381px;font-size:42px;line-height:1;letter-spacing:-.5px;font-weight:700}
.paywall__auth__main-title{display:inline}
.paywall__auth__main-title_green{color:#1b8}
.paywall__auth__body{max-width:440px;padding-right:40px;padding-bottom:40px;font-size:18px;line-height:22px;margin-left:15px}
.paywall__auth__body a{color:#1b8}
.paywall__auth__body a:hover{color:#222}
.paywall__auth__body li,.paywall__auth__body ul{list-style:disc;margin-bottom:14px}
.paywall__auth__info{font-size:14px;line-height:18px;color:#999}
.paywall__auth__info a{color:#999}
.paywall__auth__block{padding-top:60px}
.paywall__auth__title{font-family:GraphikCy-Semibold,'Helvetica CY',Arial,sans-serif;font-size:24px;line-height:26px;letter-spacing:-.29px;margin-bottom:25px}
.paywall__auth__warning{font-family:GraphikCy,'Helvetica CY',Arial,sans-serif;font-size:14px;line-height:18px;width:330px;background-color:#ffecec;padding:17px 20px;border-radius:3px;margin-bottom:25px}
.paywall__auth__warning__title{font-family:GraphikCy-Medium,'Helvetica CY',Arial,sans-serif}
.paywall__auth__warning__text{margin-top:10px}
.paywall__auth__tabs{border:1px solid #ddd;border-radius:10px;display:flex;flex-direction:row;justify-content:space-between;text-align:center;margin-bottom:30px}
.paywall__auth__tabs__item{border-radius: 10px 0 0 10px ;background:#eee ;font-size:18px;padding:13px 0;flex-basis:50%;font-family:GraphikCy-Semibold,'Helvetica CY',Arial,sans-serif;cursor:pointer}
.paywall__auth__tabs__item:first-child{border-right:1px solid #ddd;}
.paywall__auth__tabs__item.active{color:white;cursor:default;background:rgb(95,160,164);}
.paywall__auth__tabs__item.login{ border-radius: 10px 0 0 10px}
.paywall__auth__tabs__item{ border-radius: 0 10px 10px 0}
.paywall__auth__tabs__item:hover:not(.active){opacity:.7}
.paywall__auth__tab-content{display:none}
.paywall__auth__tab-content.active{display:block}
.paywall__auth__social{border-radius:3px;display:flex;flex-direction:row;justify-content:space-between;text-align:center;margin-bottom:20px}
.paywall__auth__social__link{cursor:pointer;font-size:18px;flex-basis:33.3%;font-family:GraphikCy-Semibold,'Helvetica CY',Arial,sans-serif;}
.paywall__auth__social__link:last-child{border-right:none}
.paywall__auth__social__link_vk{color:#527296;}
.paywall__auth__social__link_fb{color:#314591}
.paywall__auth__social__link_tw{color:#31adf4}
.paywall__auth__social__link_g{color:#d7443b;}
.paywall__auth__social__link:hover{opacity:.7}
.paywall__auth__form::-webkit-input-placeholder{color:#bbb}
.paywall__auth__form::-moz-placeholder{color:#bbb;opacity:1;-moz-opacity:1}
.paywall__auth__form:-ms-input-placeholder{color:#bbb}
...
.paywall__auth__form__input:-ms-input-placeholder{color:#bbb}
.paywall__auth__form__input:disabled{background:#fff}
.paywall__auth__form__submit-wrap.signup{margin-bottom:100px}
.paywall__auth__form__submit-wrap{margin-bottom:7px; border-radius: 25px}
.paywall__auth__form__submit.signup {border: 0;outline:0;cursor:pointer;display:block;width:100%;border-radius:10px 10px 10px 10px;padding:13px 0;font-size:16px;line-height:1;background:rgb(95,160,164);color:#fff;font-family:GraphikCy-Semibold,'Helvetica CY',Arial,sans-serif;}
.paywall__auth__form__submit{ border:none;outline:0;cursor:pointer;display:block;width:100%;border-radius:10px 10px 10px 10px;padding:13px 0;font-size:16px;line-height:1;background:rgb(95,160,164);color:#fff;font-family:GraphikCy-Semibold,'Helvetica CY',Arial,sans-serif;}
