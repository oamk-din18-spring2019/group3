...
input[type=number]::-webkit-outer-spin-button { 
    ...
    appearance: none;
    margin: 0; }
    .test { border-radius: 10px 10px 10px 10px;background:}
    .test2{ border-radius: 10px 10px 10px 10px;background:rgb(95,160,164)}
    .p1{background:rgba(255,255,255,0.1)}
    .paywall__auth__form__input-wrap{}
	            ...
	            <div class="paywall__auth__col paywall__auth__col_right">
	                <div class="paywall__auth__block">
	                    <div class="paywall__auth__main js-paywall-auth-main">
	                        <div class="paywall__auth__title"><h2><?= $title ?></h2></div>
...
<?php
    ...
    {
        echo '<button class="test" type="button" onclick="printInfo(\''. $method.'\',\''. $arg.'\',\''. $times[$i].'\',\''. $arrivs[$i] .'\',\''. $prices[$i].'\',\''. $passengers .'\')">'; 
        
        echo $method . ' to ' . $arg . ' departs at ' . $times[$i] . ' and costs €'. $prices[$i] . ' per passenger';
        echo '</button><br><br>';
    }
}
?>

                                        <p class="p1" id="p1"></p> <br>
<button class="paywall__auth__form__submit" id="cfm_btn" style="display:none" onclick="checkout()"></button> <br>
...
var dep, arr;
    ...
    function printInfo(cfrom, cto, timeDep, timeArr, price, passengers)
    {
        var summarytext = 'You have chosen to fly from <b>' + cfrom + '</b> to <b>' + cto + '</b>.<br><br>'
         + 'Your flight departs at <b>' + timeDep + '</b> and arrives at <b>' + timeArr + '.</b><br><br>'
         + 'You are booking for <b>' + passengers + '</b> passengers.<br><br>'
         + 'The total price is €<b>' + passengers*price + '</b>.';
        document.getElementById('p1').innerHTML = summarytext;

        document.getElementById('cfm_btn').style.display = "block";
        document.getElementById('cfm_btn').innerHTML = "Proceed to Payment";
    ...
    function checkout()
    {
        ...
        current_url = current_url.replace("flightview","");
